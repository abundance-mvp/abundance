...
public final class HouseholdItemDetector: HouseholdItemDetectorProtocol {
    ...
    /// Thread safety: Model loading is inherently thread-safe; lazy storage synchronization handled manually
    nonisolated(unsafe) private var _streamModel: VNCoreMLModel?

    private var streamModel: VNCoreMLModel? {
        if _streamModel == nil {
            ...
            _streamModel = try? VNCoreMLModel(for: mlModel)
        }
        return _streamModel
    }
